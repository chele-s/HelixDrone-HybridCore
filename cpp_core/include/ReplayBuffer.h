...
public:
  explicit SumTree(size_t capacity)
      : capacity_(capacity), treeSize_(2 * capacity - 1), tree_(treeSize_, 0.0),
        dataPointer_(0) {
  ...
  void updateBatch(const int32_t *treeIndices, const double *priorities,
  ...
  void addBatch(const double *priorities, size_t *dataIndices,
                size_t count) noexcept {
    for (size_t i = 0; i < count; ++i) {
  ...
  void get(double value, size_t &treeIdx, double &priority,
           size_t &dataIdx) const noexcept {
    size_t parentIdx = 0;
    while (true) {
      size_t leftIdx = (parentIdx << 1) + 1;
      if (leftIdx >= treeSize_)
        break;
  ...
  void sampleStratified(size_t batchSize, size_t *treeIndices,
                        double *priorities, size_t *dataIndices,
                        std::mt19937 &rng) const noexcept {
...
struct alignas(64) TransitionBlock {
  ...
  void store(size_t idx, const float *s, const float *a, float r,
             const float *ns, float d) noexcept {
  ...
  void storeBatch(size_t startIdx, const float *s, const float *a,
                  const float *r, const float *ns, const float *d,
                  size_t count) noexcept {
    for (size_t i = 0; i < count; ++i) {
      size_t idx = (startIdx + i) % capacity;
      std::memcpy(states + idx * stateDim, s + i * stateDim,
                  stateDim * sizeof(float));
      std::memcpy(actions + idx * actionDim, a + i * actionDim,
                  actionDim * sizeof(float));
      rewards[idx] = r[i];
      std::memcpy(nextStates + idx * stateDim, ns + i * stateDim,
                  stateDim * sizeof(float));
      dones[idx] = d[i];
    }
  }

  void gather(const size_t *indices, size_t count, float *outStates,
              float *outActions, float *outRewards, float *outNextStates,
              float *outDones) const noexcept {
    for (size_t i = 0; i < count; ++i) {
      size_t idx = indices[i];
      std::memcpy(outStates + i * stateDim, states + idx * stateDim,
                  stateDim * sizeof(float));
      std::memcpy(outActions + i * actionDim, actions + idx * actionDim,
                  actionDim * sizeof(float));
      outRewards[i] = rewards[idx];
      std::memcpy(outNextStates + i * stateDim, nextStates + idx * stateDim,
                  stateDim * sizeof(float));
...
public:
  PrioritizedReplayBuffer(size_t capacity, size_t stateDim, size_t actionDim,
                          double alpha = 0.6, double betaStart = 0.4,
                          size_t betaFrames = 100000, double epsilon = 1e-6)
      : capacity_(capacity), stateDim_(stateDim), actionDim_(actionDim),
        alpha_(alpha), betaStart_(betaStart), betaFrames_(betaFrames),
        epsilon_(epsilon), frame_(1), size_(0), maxPriority_(1.0),
        tree_(capacity), data_(capacity, stateDim, actionDim),
        rng_(std::random_device{}()), tempTreeIndices_(capacity),
        tempPriorities_(capacity), tempDataIndices_(capacity) {}

  double beta() const noexcept {
    double progress =
        static_cast<double>(frame_) / static_cast<double>(betaFrames_);
  ...
  void pushBatch(const float *states, const float *actions,
                 const float *rewards, const float *nextStates,
                 const float *dones, size_t batchSize) noexcept {
    double priority = std::pow(maxPriority_, alpha_);

    for (size_t i = 0; i < batchSize; ++i) {
      size_t dataIdx = tree_.add(priority);
      data_.store(dataIdx, states + i * stateDim_, actions + i * actionDim_,
                  rewards[i], nextStates + i * stateDim_, dones[i]);
    }
  ...
  SampleResult sample(size_t batchSize) {
    ...
    tree_.sampleStratified(batchSize, tempTreeIndices_.data(),
                           tempPriorities_.data(), tempDataIndices_.data(),
                           rng_);

    data_.gather(tempDataIndices_.data(), batchSize, result.states.data(),
                 result.actions.data(), result.rewards.data(),
                 result.nextStates.data(), result.dones.data());

    double totalP = std::max(tree_.totalPriority(), 1e-8);
    double currentBeta = beta();
    double sizeD = static_cast<double>(size_);

    computeISWeights(tempPriorities_.data(), result.weights.data(), batchSize,
                     totalP, sizeD, currentBeta);
  ...
  void updatePriorities(const int32_t *treeIndices, const double *tdErrors,
                        size_t count) noexcept {
...
public:
  UniformReplayBuffer(size_t capacity, size_t stateDim, size_t actionDim)
      : capacity_(capacity), stateDim_(stateDim), actionDim_(actionDim),
        ptr_(0), size_(0), data_(capacity, stateDim, actionDim),
  ...
  void pushBatch(const float *states, const float *actions,
                 const float *rewards, const float *nextStates,
                 const float *dones, size_t batchSize) noexcept {
    data_.storeBatch(ptr_, states, actions, rewards, nextStates, dones,
                     batchSize);
  ...
  SampleResult sample(size_t batchSize) {
    ...
    data_.gather(indices.data(), batchSize, result.states.data(),
                 result.actions.data(), result.rewards.data(),
                 result.nextStates.data(), result.dones.data());
...
struct alignas(64) SequenceBlock {
  ...
  void store(size_t idx, const float *obs, const float *act, float rew,
    ...
    std::memcpy(next_observations + idx * obs_dim, next_obs,
  ...
  void gatherSequences(const size_t *start_indices, size_t batch_size,
                       size_t seq_len, float *out_obs, float *out_next_obs,
                       float *out_actions, float *out_rewards,
                       float *out_dones) const noexcept {
    for (size_t b = 0; b < batch_size; ++b) {
      for (size_t t = 0; t < seq_len; ++t) {
        size_t idx = (start_indices[b] + t) % capacity;
        size_t out_idx = b * seq_len + t;
        std::memcpy(out_obs + out_idx * obs_dim, observations + idx * obs_dim,
                    obs_dim * sizeof(float));
        std::memcpy(out_next_obs + out_idx * obs_dim,
                    next_observations + idx * obs_dim, obs_dim * sizeof(float));
        std::memcpy(out_actions + out_idx * action_dim,
                    actions + idx * action_dim, action_dim * sizeof(float));
...
public:
  ...
  SequenceReplayBuffer(size_t capacity, size_t obs_dim, size_t action_dim,
                       size_t seq_len, size_t burn_in_len = 0)
      : capacity_(capacity), obs_dim_(obs_dim), action_dim_(action_dim),
        seq_len_(seq_len), burn_in_len_(burn_in_len),
        total_seq_len_(seq_len + burn_in_len), ptr_(0), size_(0),
        valid_count_(0), current_episode_id_(0),
        data_(capacity, obs_dim, action_dim), rng_(std::random_device{}()),
  ...
  void pushBatch(const float *obs, const float *actions, const float *rewards,
                 const float *next_obs, const float *dones,
                 size_t count) noexcept {
    for (size_t i = 0; i < count; ++i) {
  ...
  SampleResult sample(size_t batch_size) {
    ...
    if (valid_count_ == 0) {
      ...
      if (burn_in_len_ > 0) {
        ...
        result.burn_in_next_obs.resize(batch_size * burn_in_len_ * obs_dim_,
                                       0.0f);
        result.burn_in_actions.resize(batch_size * burn_in_len_ * action_dim_,
                                      0.0f);
    ...
    if (collected < batch_size) {
      for (size_t i = collected; i < batch_size; ++i) {
        result.start_indices[i] =
            result.start_indices[i % std::max(collected, size_t(1))];
      }
    }

    data_.gatherSequences(result.start_indices.data(), batch_size,
                          total_seq_len_, full_obs.data(), full_next_obs.data(),
                          full_actions.data(), full_rewards.data(),
                          full_dones.data());
  ...
  void endEpisode() noexcept { ++current_episode_id_; }

  size_t size() const noexcept { return size_; }
  size_t capacity() const noexcept { return capacity_; }
  size_t burnInLength() const noexcept { return burn_in_len_; }
  bool isReady(size_t batch_size) const noexcept {
    return valid_count_ >= batch_size;
  }
...
private:
  ...
  bool isValidSequence(size_t start) const noexcept {
    ...
    if (data_.episode_ids[start] != data_.episode_ids[end])
      return false;
    for (size_t t = 0; t < total_seq_len_ - 1; ++t) {
      size_t idx = (start + t) % capacity_;
      if (data_.dones[idx] > 0.5f)
        return false;
    }
    return true;
  }
  ...
  void setBit(size_t idx) noexcept {
    valid_mask_[idx / 64] |= (1ULL << (idx % 64));
  }
  void clearBit(size_t idx) noexcept {
    valid_mask_[idx / 64] &= ~(1ULL << (idx % 64));
  }

  size_t capacity_, obs_dim_, action_dim_, seq_len_, burn_in_len_,
      total_seq_len_;
...
public:
  ...
  SequencePrioritizedReplayBuffer(size_t capacity, size_t obs_dim,
                                  size_t action_dim, size_t seq_len,
                                  size_t burn_in_len = 0, double alpha = 0.6,
                                  double beta_start = 0.4,
                                  size_t beta_frames = 100000,
                                  double epsilon = 1e-6)
      : capacity_(capacity), obs_dim_(obs_dim), action_dim_(action_dim),
        seq_len_(seq_len), burn_in_len_(burn_in_len),
        total_seq_len_(seq_len + burn_in_len), alpha_(alpha),
        beta_start_(beta_start), beta_frames_(beta_frames), epsilon_(epsilon),
        frame_(1), ptr_(0), size_(0), valid_count_(0), current_episode_id_(0),
        max_priority_(1.0), data_(capacity, obs_dim, action_dim),
        priorities_(capacity, 0.0), rng_(std::random_device{}()),
        valid_mask_((capacity + 63) / 64, 0) {}

  double beta() const noexcept {
    double progress =
        static_cast<double>(frame_) / static_cast<double>(beta_frames_);
  ...
  void updatePriorities(const size_t *indices, const double *td_errors,
                        size_t count) noexcept {
    for (size_t i = 0; i < count; ++i) {
      ...
      while (priority > current_max &&
             !max_priority_.compare_exchange_weak(current_max, priority))
        ;
    }
  }
